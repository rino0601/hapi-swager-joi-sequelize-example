    ...
    config: {
        handler: (request, reply) => {
            db.User.findById(request.params.id).then(userOrNull=>{
                ...
                return user.getProfile().then(profileOrNull=>{
                    if(profileOrNull === null) throw Error("Not Found");
                    return user.setProfile(null).then(()=>profileOrNull.reload()).then(profile=>{
                        reply(profile).code(200);    
                    });
                });
            }).catch(err => {
